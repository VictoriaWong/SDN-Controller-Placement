...
def theano_expression(setting, Pv_vec, capacity, latency, decayFactor, isGradient=True):
    ...
    if isGradient:
        g = T.grad(obj, P_red)
        gradient = function(inputs=[P_red, lamda, capa, D, dF, delta, beta, mu1, mu2, mu3], outputs=[g], on_unused_input='warn')
        return gradient(Pv_vec, setting.arrivalRate, capacity, latency, decayFactor, setting.dlt, setting.beta, setting.mu[0], setting.mu[1], setting.mu[2])
    else:
        cost = function(inputs=[P_red, lamda, capa, D, dF, delta, beta, mu1, mu2, mu3], outputs=[obj], on_unused_input='warn')
        return cost(Pv_vec, setting.arrivalRate, capacity, latency, decayFactor, setting.dlt, setting.beta, setting.mu[0], setting.mu[1], setting.mu[2])
